...
- **核心功能**:
  ...
  - 在线预测系统：基于离线训练模型,通过点选式表单输入客户特征,预测认购意愿
...
│   └── conftest.py
├── data/                      # 数据目录
│   ├── train.csv              # 训练数据 (2.8 MB)
│   └── test.csv               # 测试数据 (0.9 MB)
├── requirements.txt            # 生产运行依赖
├── requirements-dev.txt        # 本地/CI 检查依赖
├── Dockerfile                  # 容器镜像定义
├── .github/workflows/
│   └── ci.yml                 # 持续集成（仅 CI，无 CD）
├── .gitignore
├── README.md
...
- CI 红灯不合并。
- 模型训练是离线操作（本地执行训练脚本产生模型文件），预测服务加载已训练模型。**Docker 构建时将模型训练一并完成，确保镜像自包含。**
- **本项目仅做 CI，不做 CD**。本地部署验证：`docker build` + `docker run`，启动后提供 URL 给用户验证。
...
| `<PORT>` | `8888` | 服务端口（Streamlit 默认 8501，映射到主机 8888） |
